...
var x >=0;
var y >=0;
var S{b in BEAMS} >=0, <=40; #strength of beam n
...
var ofs{l in LVL, i in ROWS, j in COLUMNS} >= 0;
# Pushing all variables to the maximum value of their corresponding indices

#minimize beamusage: sum{b in BEAMS} (sum{l in LVL} (sum {i in ROWS, j in COLUMNS} S[b]*matrix_value[b*num_lvl+l,i,j])); #use this objective function to find minimum total dosage
minimize offs: sum{l in LVL, i in ROWS, j in COLUMNS} ofs[l, i, j];
# Each variable at an index is >= to the maximum value at 
# the index across all matrices given.

subject to MinReq {(l,i,j) in TUMOR}: 
	sum{b in BEAMS} (S[b] * matrix_value[(b*num_lvl+l),i,j]) >= min_dose-ofs[l,i,j];

subject to MaxReq {(l,i,j) in CRIT}: 
	sum{b in BEAMS} (S[b] * matrix_value[(b*num_lvl+l),i,j]) <= max_dose;
